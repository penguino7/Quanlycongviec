...
const initialFinanceData = [
  { id: '1', date: '2026-03-01', category: 'Lương', type: 'income', amount: 25000000, note: 'Lương tháng 2' },
  { id: '2', date: '2026-03-02', category: 'Ăn uống', type: 'expense', amount: 150000, note: 'Ăn trưa' },
  { id: '3', date: '2026-03-05', category: 'Di chuyển', type: 'expense', amount: 500000, note: 'Xăng xe & Gửi xe' },
  { id: '4', date: '2026-03-10', category: 'Mua sắm', type: 'expense', amount: 2500000, note: 'Mua quần áo' },
  { id: '5', date: '2026-03-12', category: 'Tiền thưởng', type: 'income', amount: 5000000, note: 'Thưởng dự án' },
  { id: '6', date: '2026-03-14', category: 'Ăn uống', type: 'expense', amount: 1200000, note: 'Tiệc sinh nhật' },
  { id: '7', date: '2026-03-15', category: 'Tiền điện', type: 'expense', amount: 850000, note: 'Điện tháng 2' },
  { id: '8', date: '2026-03-16', category: 'Ăn uống', type: 'expense', amount: 200000, note: 'Cà phê họp nhóm' },
  { id: '9', date: '2026-03-17', category: 'Mua sắm', type: 'expense', amount: 1500000, note: 'Mua chuột mới' },
];
...
export const Finance = () => {
  ...
  const categoryData = useMemo(() => {
    ...
    data.filter(d => d.type === 'expense').forEach(curr => {
  ...
  const StatCard = ({ title, value, icon, trend, color }: any) => (
    <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 transition-all hover:shadow-md group">
      <div className="flex justify-between items-start mb-4">
        <div className={`p-3 rounded-xl bg-opacity-10 dark:bg-opacity-20 ${color.bg}`}>
          {icon}
        </div>
        {trend && (
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard 
          ...
          trend={-5}
          color={{ bg: 'bg-rose-500', text: 'text-rose-600' }}
        />
        <StatCard 
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-slate-800">
                  ...
                  <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-gray-900 dark:text-white">{format(parseISO(item.date), 'MMM dd, yyyy')}</span>
                        <span className="text-[10px] text-gray-400 font-medium uppercase tracking-widest">{format(parseISO(item.date), 'EEEE')}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-gray-900 dark:text-white group-hover:text-indigo-600 transition-colors">{item.category}</p>
                          <p className="text-xs text-gray-500 dark:text-gray-400 truncate max-w-[150px]">{item.note}</p>
                        </div>
                      </div>
                    </td>
                ...
                {filteredList.length === 0 && (
                  <tr>
                    <td colSpan={3} className="px-6 py-12 text-center text-gray-400 italic">No transactions found.</td>
                  </tr>
                )}
              </tbody>
            </table>
